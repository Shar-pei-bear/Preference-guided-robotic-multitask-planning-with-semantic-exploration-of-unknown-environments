...
class Planner:
    ...
    def save_data(self):
        with open("/media/bear/T7/Toys/Partial_Knowledge/Metric.pkl", "wb") as fp:  # Pickling
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.obstacle_map is not None and not self.initial_obs and not self.mission_status:
                ...
                task_prob = upper_bound(probs, self.gains, temp_dfa_state)
                #task_prob = self.task_1_reward*task1_prob + self.task_2_reward*task2_prob + self.task_3_reward*task3_prob
                ...
                if not self.goal_status:
                    ...
                    self.goal_status =  self.dfa_state[0] or (self.dfa_state[5] == 2 and not self.reset)
                ...
                if self.goal_status:
                    ...
                    if goal_phase is not None:
                        ...
                        if not self.ang_goal1_status:
                            ...
                            command_msg.linear_vel = 0
                            command_msg.angular_vel = 0
                    elif self.dfa[5].state == 2 and not self.reset:
                        # if self.goal is None:
                        self.goal = np.asarray([0, 0])
                        to_waypoint = self.goal - self.pos
                        angle_error = np.arctan2(to_waypoint[1], to_waypoint[0]) - self.ang
                        angle_error = np.arctan2(np.sin(angle_error),
                                                 np.cos(angle_error))
                        ...
                        if np.linalg.norm(to_waypoint) < 0.2 and np.abs(angle_error) < 0.2:
                            self.reset = True
                            print('current_planning time is ', self.planning_time)
                            command_msg.linear_vel = 0
                            command_msg.angular_vel = 0
