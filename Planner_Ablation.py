...
from upper_bound import upper_bound_new
import pickle
...
class Planner:
    ...
    def save_data(self):
        with open("/media/bear/T7/Toys/Replanning_MDP/Metric.pkl", "wb") as fp:  # Pickling
            ...
            if self.obstacle_map is not None and not self.initial_obs and not self.mission_status:
                ...
                if self.old_grip_maps and self.old_goal_map is not None and self.old_grid_map is not None and self.grid_map_grey[pos_grid[1], pos_grid[0]] == 255:
                    ...
                    for old_grip_map in self.old_grip_maps.values():
                        if old_grip_map.shape != self.old_goal_map.shape:
                            continue

                    bound_start_time = time()
                    task_prob = upper_bound_new(probs, self.gains, self.dfa_state[1:6], np.flipud(pos_grid), self.dfa_states,
                                    self.dfa_states_indices, self.old_grid_map, self.old_grip_maps,
                                    self.old_goal_map, self.action_deltas)
                    bound_end_time = time()
