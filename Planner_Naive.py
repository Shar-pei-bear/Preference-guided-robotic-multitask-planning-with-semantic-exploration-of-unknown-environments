...
class Planner:
    ...
    def save_data(self):
        # with open("/media/bear/T7/Toys/TwoTasks/TaskProbs.pkl", "wb") as fp:  # Pickling
        #     pickle.dump([self.log_1, self.log_2, self.log_3], fp)
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.obstacle_map is not None and not self.initial_obs and not self.mission_status:
                ...
                start_time = time()
                task1_prob = self.max_class_probs.get('car', 0) * self.max_class_probs.get('train', 0)
                task2_prob = self.max_class_probs.get('parasaurolophus', 0) * self.max_class_probs.get('stegosaurus', 0)
                task3_prob = self.max_class_probs.get('tank', 0)

                task_prob = self.task_1_reward*task1_prob + self.task_2_reward*task2_prob + self.task_3_reward*task3_prob

                if  (task_prob - self.old_task_prob) > 0.5 and (task_prob - self.prev_task_prob) < 0.01: # 0.5
                ...
                self.log_1.append(task1_prob)
                self.log_2.append(task2_prob)
                self.log_3.append(task3_prob)
    ...
    def find_sightings(self):
        ...
        for obj_id, obj in self.objects.items():
            ...
            for target_class in self.target_classes:
                ...
                if obj.class_probs[target_class_index] > self.max_class_probs.get(target_class, 0):
                    self.max_class_probs[target_class] = obj.class_probs[target_class_index]
